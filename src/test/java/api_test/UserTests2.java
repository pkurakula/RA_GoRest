...
import api_endpoints.EndPoints2;
...
public class UserTests2 {
	...
	public void testGetUser() {
		...
		Response response = EndPoints2.getUser(id);
		response.then().log().all();
	...
	public void testUpdateUser() {
		...
		Response response = EndPoints2.updateUser(userPayload, id);
		response.then().log().all();
		Assert.assertEquals(response.getStatusCode(), 200);
	...
	public void testDeleteUser() {
		...
		Response response = EndPoints2.deleteUser(id);
		response.then().log().all();
